...
def plot_heightmaps():
    ...
    # Initialize the MeshBox (which generates the 6 poses)
    ...
    for i, pose in enumerate(box.poses):
        ...
        # Replace np.inf with NaN in Hb so it plots correctly (background)
        Hb_display = Hb.copy()
        Hb_display[Hb_display == np.inf] = np.nan
        
        # Replace 0 with NaN in Ht if it didn't hit anything (background)
        Ht_display = Ht.copy()
